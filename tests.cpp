...
TEST(modulepp, LoadTestLibrary) {
  ...
  auto* m = ModuleLoader::loadModule<TestModule>(TEST_LIBRARY_PATH);
...
TEST(modulepp, LoadNotExistentLibrary) {
  ...
  auto* m = ModuleLoader::loadModule<TestModule>(NON_EXISTENT_LIBRARY_PATH);
  EXPECT_EQ(m, nullptr);
}

TEST(modulepp, LoadBadModule) {
  EXPECT_TRUE(std::filesystem::exists(BAD_MODULE_LIBRARY_PATH));
  auto* m = ModuleLoader::loadModule<BadModule>(BAD_MODULE_LIBRARY_PATH);
  EXPECT_EQ(m, nullptr);
}

TEST(modulepp, LoadDirectory) {
  auto modules = ModuleLoader::loadDirectory<TestModule>(std::filesystem::current_path());
